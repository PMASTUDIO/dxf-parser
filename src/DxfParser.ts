...
export default class DxfParser {
  ...
  private _parse(dxfString: string) {
      ...
      while (curr.value !== "EOF") {
        ...
        if (groupIs(curr, 0, "TABLE")) {
          ...
          const tableDefinition =
            tableDefinitions[curr.value as keyof ITableDefinitions];
          if (tableDefinition) {
            log.debug(curr.value + " Table {");
            tables[
              tableDefinitions[curr.value as keyof ITableDefinitions].tableName
            ] = parseTable(curr);
            log.debug("}");
          } else {
    ...
    function parseTable<T extends IBaseTable = ITable>(group: IGroup) {
      const tableDefinition =
        tableDefinitions[group.value as keyof ITableDefinitions];
      const table = {} as T;
      let expectedCount = 0;
